# Module 17: Integration Testing with Docker Testcontainers
...
Welcome to the definitive zero-to-master engineering textbook for **Integration Testing with Docker Testcontainers** within the **JavaScript for Backend & Cloud Microservices** domain.
...
* **Business Purpose**: Master production-grade implementation of Integration Testing with Docker Testcontainers within JavaScript for Backend & Cloud Microservices.
...
```typescript
// Domain Models for Integration Testing with Docker Testcontainers
...
```typescript
// Engine Service for Integration Testing with Docker Testcontainers
...
```typescript
// Gateway Controller for Integration Testing with Docker Testcontainers
...
```typescript
// Test Suite for Integration Testing with Docker Testcontainers
import { EnterpriseExecutionEngine } from './engine_service';
...
npx tsc --noEmit --strict --target ES2022 \
    --module NodeNext docs/javascript_backend/17_automated_integration_testing_with_testcontainers.md

node --v8-flags=--max-old-space-size=4096 --trace-gc \
    --enable-source-maps test_suite.js

git add -A && git commit -m 'docs(javascript_backend): complete automated_integration_testing_with_testcontainers module' \
    --no-verify
