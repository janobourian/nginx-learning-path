# Module 3: Impeller 3D Graphics Engine: Metal and Vulkan
Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: Impeller 3D Graphics Engine: Metal and Vulkan

### Concept
Welcome to Impeller 3D Graphics Engine: Metal and Vulkan. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
|-----|-----------|-------------|
| `Impeller` | `void Impeller(...)` | Essential operation for managing `Impeller` within the Flutter framework. |
| `Skia` | `void Skia(...)` | Essential operation for managing `Skia` within the Flutter framework. |
| `Metal` | `void Metal(...)` | Essential operation for managing `Metal` within the Flutter framework. |
| `Vulkan` | `void Vulkan(...)` | Essential operation for managing `Vulkan` within the Flutter framework. |
| `Aiks` | `void Aiks(...)` | Essential operation for managing `Aiks` within the Flutter framework. |
| `Entity` | `void Entity(...)` | Essential operation for managing `Entity` within the Flutter framework. |
| `Contents` | `void Contents(...)` | Essential operation for managing `Contents` within the Flutter framework. |
| `Renderer` | `void Renderer(...)` | Essential operation for managing `Renderer` within the Flutter framework. |
| `Context` | `void Context(...)` | Essential operation for managing `Context` within the Flutter framework. |
| `Command Buffer` | `void Command Buffer(...)` | Essential operation for managing `Command Buffer` within the Flutter framework. |
| `Render Pass` | `void Render Pass(...)` | Essential operation for managing `Render Pass` within the Flutter framework. |
| `Pipeline State Object (PSO)` | `void Pipeline State Object (PSO)(...)` | Essential operation for managing `Pipeline State Object (PSO)` within the Flutter framework. |
| `Vertex Shader` | `void Vertex Shader(...)` | Essential operation for managing `Vertex Shader` within the Flutter framework. |
| `Fragment Shader` | `void Fragment Shader(...)` | Essential operation for managing `Fragment Shader` within the Flutter framework. |
| `Tessellation` | `void Tessellation(...)` | Essential operation for managing `Tessellation` within the Flutter framework. |
| `MSAA` | `void MSAA(...)` | Essential operation for managing `MSAA` within the Flutter framework. |
| `flutter run --enable-impeller` | `void flutter run --enable-impeller(...)` | Essential operation for managing `flutter run --enable-impeller` within the Flutter framework. |
| `DisplayList` | `void DisplayList(...)` | Essential operation for managing `DisplayList` within the Flutter framework. |
| `flutter build apk --enable-impeller` | `void flutter build apk --enable-impeller(...)` | Essential operation for managing `flutter build apk --enable-impeller` within the Flutter framework. |
| `Info.plist FLTEnableImpeller` | `void Info_plist FLTEnableImpeller(...)` | Essential operation for managing `Info.plist FLTEnableImpeller` within the Flutter framework. |
...
To further understand the intricacies of Impeller 3D Graphics Engine: Metal and Vulkan, let's examine additional architectural patterns.
