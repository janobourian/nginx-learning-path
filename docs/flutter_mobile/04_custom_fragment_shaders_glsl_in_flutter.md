# Module 4: Custom Fragment Shaders: GLSL in Flutter

Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: Custom Fragment Shaders: GLSL in Flutter

### Concept

Welcome to Custom Fragment Shaders: GLSL in Flutter. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
| ----- | ----------- | ------------- |
| `FragmentProgram.fromAsset()` | `void FragmentProgram_fromAsset(...)` | Essential operation for managing `FragmentProgram.fromAsset()` within the Flutter framework. |
| `FragmentShader` | `void FragmentShader(...)` | Essential operation for managing `FragmentShader` within the Flutter framework. |
| `Canvas.drawRect()` | `void Canvas_drawRect(...)` | Essential operation for managing `Canvas.drawRect()` within the Flutter framework. |
| `Paint().shader` | `void Paint_shader(...)` | Essential operation for managing `Paint().shader` within the Flutter framework. |
| `shader.setFloat()` | `void shader_setFloat(...)` | Essential operation for managing `shader.setFloat()` within the Flutter framework. |
| `shader.setImageSampler()` | `void shader_setImageSampler(...)` | Essential operation for managing `shader.setImageSampler()` within the Flutter framework. |
| `AnimatedSampler` | `void AnimatedSampler(...)` | Essential operation for managing `AnimatedSampler` within the Flutter framework. |
| `ui.Image` | `void ui_Image(...)` | Essential operation for managing `ui.Image` within the Flutter framework. |
| `ui.FragmentProgram` | `void ui_FragmentProgram(...)` | Essential operation for managing `ui.FragmentProgram` within the Flutter framework. |
| `GLSL` | `void GLSL(...)` | Essential operation for managing `GLSL` within the Flutter framework. |
| `vec2` | `void vec2(...)` | Essential operation for managing `vec2` within the Flutter framework. |
| `vec3` | `void vec3(...)` | Essential operation for managing `vec3` within the Flutter framework. |
| `vec4` | `void vec4(...)` | Essential operation for managing `vec4` within the Flutter framework. |
| `mat3` | `void mat3(...)` | Essential operation for managing `mat3` within the Flutter framework. |
| `mat4` | `void mat4(...)` | Essential operation for managing `mat4` within the Flutter framework. |
| `texture2D` | `void texture2D(...)` | Essential operation for managing `texture2D` within the Flutter framework. |
| `gl_FragCoord` | `void gl_FragCoord(...)` | Essential operation for managing `gl_FragCoord` within the Flutter framework. |
| `gl_FragColor` | `void gl_FragColor(...)` | Essential operation for managing `gl_FragColor` within the Flutter framework. |
| `uniform` | `void uniform(...)` | Essential operation for managing `uniform` within the Flutter framework. |
| `varying` | `void varying(...)` | Essential operation for managing `varying` within the Flutter framework. |
...
To further understand the intricacies of Custom Fragment Shaders: GLSL in Flutter, let's examine additional architectural patterns.
