# Module 6: State Management with BLoC and Cubit Streams

Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: State Management with BLoC and Cubit Streams

### Concept

Welcome to State Management with BLoC and Cubit Streams. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
| ----- | ----------- | ------------- |
| `Bloc<Event, State>` | `void Bloc<Event, State>(...)` | Essential operation for managing `Bloc<Event, State>` within the Flutter framework. |
| `Cubit<State>` | `void Cubit<State>(...)` | Essential operation for managing `Cubit<State>` within the Flutter framework. |
| `BlocBuilder` | `void BlocBuilder(...)` | Essential operation for managing `BlocBuilder` within the Flutter framework. |
| `BlocListener` | `void BlocListener(...)` | Essential operation for managing `BlocListener` within the Flutter framework. |
| `BlocProvider` | `void BlocProvider(...)` | Essential operation for managing `BlocProvider` within the Flutter framework. |
| `MultiBlocProvider` | `void MultiBlocProvider(...)` | Essential operation for managing `MultiBlocProvider` within the Flutter framework. |
| `emit()` | `void emit(...)` | Essential operation for managing `emit()` within the Flutter framework. |
| `on<Event>()` | `void on<Event>(...)` | Essential operation for managing `on<Event>()` within the Flutter framework. |
| `BlocObserver` | `void BlocObserver(...)` | Essential operation for managing `BlocObserver` within the Flutter framework. |
| `onChange()` | `void onChange(...)` | Essential operation for managing `onChange()` within the Flutter framework. |
| `onTransition()` | `void onTransition(...)` | Essential operation for managing `onTransition()` within the Flutter framework. |
| `onError()` | `void onError(...)` | Essential operation for managing `onError()` within the Flutter framework. |
| `RepositoryProvider` | `void RepositoryProvider(...)` | Essential operation for managing `RepositoryProvider` within the Flutter framework. |
| `BlocSelector` | `void BlocSelector(...)` | Essential operation for managing `BlocSelector` within the Flutter framework. |
| `context.read<T>()` | `void context_read<T>(...)` | Essential operation for managing `context.read<T>()` within the Flutter framework. |
| `context.watch<T>()` | `void context_watch<T>(...)` | Essential operation for managing `context.watch<T>()` within the Flutter framework. |
| `context.select<T, R>()` | `void context_select<T, R>(...)` | Essential operation for managing `context.select<T, R>()` within the Flutter framework. |
...
| `Equatable` | `void Equatable(...)` | Essential operation for managing `Equatable` within the Flutter framework. |
| `Freezed` | `void Freezed(...)` | Essential operation for managing `Freezed` within the Flutter framework. |
...
To further understand the intricacies of State Management with BLoC and Cubit Streams, let's examine additional architectural patterns.
