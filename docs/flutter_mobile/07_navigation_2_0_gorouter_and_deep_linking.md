# Module 7: Navigation 2.0: GoRouter and Deep Linking
Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: Navigation 2.0: GoRouter and Deep Linking

### Concept
Welcome to Navigation 2.0: GoRouter and Deep Linking. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
|-----|-----------|-------------|
| `GoRouter` | `void GoRouter(...)` | Essential operation for managing `GoRouter` within the Flutter framework. |
| `GoRoute` | `void GoRoute(...)` | Essential operation for managing `GoRoute` within the Flutter framework. |
| `ShellRoute` | `void ShellRoute(...)` | Essential operation for managing `ShellRoute` within the Flutter framework. |
| `StatefulShellRoute` | `void StatefulShellRoute(...)` | Essential operation for managing `StatefulShellRoute` within the Flutter framework. |
| `GoRouterState` | `void GoRouterState(...)` | Essential operation for managing `GoRouterState` within the Flutter framework. |
| `context.go()` | `void context_go(...)` | Essential operation for managing `context.go()` within the Flutter framework. |
| `context.push()` | `void context_push(...)` | Essential operation for managing `context.push()` within the Flutter framework. |
| `context.pop()` | `void context_pop(...)` | Essential operation for managing `context.pop()` within the Flutter framework. |
| `context.replace()` | `void context_replace(...)` | Essential operation for managing `context.replace()` within the Flutter framework. |
| `context.goNamed()` | `void context_goNamed(...)` | Essential operation for managing `context.goNamed()` within the Flutter framework. |
| `context.pushNamed()` | `void context_pushNamed(...)` | Essential operation for managing `context.pushNamed()` within the Flutter framework. |
| `Universal Links` | `void Universal Links(...)` | Essential operation for managing `Universal Links` within the Flutter framework. |
| `App Links` | `void App Links(...)` | Essential operation for managing `App Links` within the Flutter framework. |
| `assetlinks.json` | `void assetlinks_json(...)` | Essential operation for managing `assetlinks.json` within the Flutter framework. |
| `apple-app-site-association` | `void apple-app-site-association(...)` | Essential operation for managing `apple-app-site-association` within the Flutter framework. |
| `DeepLink` | `void DeepLink(...)` | Essential operation for managing `DeepLink` within the Flutter framework. |
| `RouteMatch` | `void RouteMatch(...)` | Essential operation for managing `RouteMatch` within the Flutter framework. |
| `Redirect` | `void Redirect(...)` | Essential operation for managing `Redirect` within the Flutter framework. |
| `onException` | `void onException(...)` | Essential operation for managing `onException` within the Flutter framework. |
| `navigatorKey` | `void navigatorKey(...)` | Essential operation for managing `navigatorKey` within the Flutter framework. |
...
To further understand the intricacies of Navigation 2.0: GoRouter and Deep Linking, let's examine additional architectural patterns.
