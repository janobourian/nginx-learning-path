# Module 11: Secure Storage and Biometric Authentication
Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: Secure Storage and Biometric Authentication

### Concept
Welcome to Secure Storage and Biometric Authentication. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
|-----|-----------|-------------|
| `flutter_secure_storage` | `void flutter_secure_storage(...)` | Essential operation for managing `flutter_secure_storage` within the Flutter framework. |
| `FlutterSecureStorage()` | `void FlutterSecureStorage(...)` | Essential operation for managing `FlutterSecureStorage()` within the Flutter framework. |
| `read()` | `void read(...)` | Essential operation for managing `read()` within the Flutter framework. |
| `write()` | `void write(...)` | Essential operation for managing `write()` within the Flutter framework. |
| `delete()` | `void delete(...)` | Essential operation for managing `delete()` within the Flutter framework. |
| `deleteAll()` | `void deleteAll(...)` | Essential operation for managing `deleteAll()` within the Flutter framework. |
| `IOSOptions` | `void IOSOptions(...)` | Essential operation for managing `IOSOptions` within the Flutter framework. |
| `AndroidOptions` | `void AndroidOptions(...)` | Essential operation for managing `AndroidOptions` within the Flutter framework. |
| `EncryptedSharedPreferences` | `void EncryptedSharedPreferences(...)` | Essential operation for managing `EncryptedSharedPreferences` within the Flutter framework. |
| `Keychain` | `void Keychain(...)` | Essential operation for managing `Keychain` within the Flutter framework. |
| `local_auth` | `void local_auth(...)` | Essential operation for managing `local_auth` within the Flutter framework. |
| `LocalAuthentication` | `void LocalAuthentication(...)` | Essential operation for managing `LocalAuthentication` within the Flutter framework. |
| `canCheckBiometrics` | `void canCheckBiometrics(...)` | Essential operation for managing `canCheckBiometrics` within the Flutter framework. |
| `isDeviceSupported()` | `void isDeviceSupported(...)` | Essential operation for managing `isDeviceSupported()` within the Flutter framework. |
| `getAvailableBiometrics()` | `void getAvailableBiometrics(...)` | Essential operation for managing `getAvailableBiometrics()` within the Flutter framework. |
| `authenticate()` | `void authenticate(...)` | Essential operation for managing `authenticate()` within the Flutter framework. |
| `BiometricType.face` | `void BiometricType_face(...)` | Essential operation for managing `BiometricType.face` within the Flutter framework. |
| `BiometricType.fingerprint` | `void BiometricType_fingerprint(...)` | Essential operation for managing `BiometricType.fingerprint` within the Flutter framework. |
| `AuthenticationOptions` | `void AuthenticationOptions(...)` | Essential operation for managing `AuthenticationOptions` within the Flutter framework. |
| `StickyAuth` | `void StickyAuth(...)` | Essential operation for managing `StickyAuth` within the Flutter framework. |
...
To further understand the intricacies of Secure Storage and Biometric Authentication, let's examine additional architectural patterns.
