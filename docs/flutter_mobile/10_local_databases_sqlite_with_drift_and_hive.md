# Module 10: Local Databases: SQLite with Drift and Hive

Track 12: Flutter for Mobile & Impeller Graphics Engine

## 1. Opening: Local Databases: SQLite with Drift and Hive

### Concept

Welcome to Local Databases: SQLite with Drift and Hive. In the world of Flutter, understanding this concept is critical for building high-performance, robust mobile applications. Flutter represents a fundamental shift in mobile app development by drawing its own UI components directly to the screen via a highly optimized engine (like Impeller), rather than wrapping OEM platform widgets.
...
| ----- | ----------- | ------------- |
| `@DriftDatabase` | `void DriftDatabase(...)` | Essential operation for managing `@DriftDatabase` within the Flutter framework. |
| `Table` | `void Table(...)` | Essential operation for managing `Table` within the Flutter framework. |
| `DataClass` | `void DataClass(...)` | Essential operation for managing `DataClass` within the Flutter framework. |
| `@UseRowClass` | `void UseRowClass(...)` | Essential operation for managing `@UseRowClass` within the Flutter framework. |
| `select()` | `void select(...)` | Essential operation for managing `select()` within the Flutter framework. |
| `transaction()` | `void transaction(...)` | Essential operation for managing `transaction()` within the Flutter framework. |
| `insert()` | `void insert(...)` | Essential operation for managing `insert()` within the Flutter framework. |
| `update()` | `void update(...)` | Essential operation for managing `update()` within the Flutter framework. |
| `delete()` | `void delete(...)` | Essential operation for managing `delete()` within the Flutter framework. |
| `watch()` | `void watch(...)` | Essential operation for managing `watch()` within the Flutter framework. |
| `LazyDatabase` | `void LazyDatabase(...)` | Essential operation for managing `LazyDatabase` within the Flutter framework. |
| `NativeDatabase` | `void NativeDatabase(...)` | Essential operation for managing `NativeDatabase` within the Flutter framework. |
| `Hive.openBox('name')` | `void Hive_openBox('name')(...)` | Essential operation for managing `Hive.openBox('name')` within the Flutter framework. |
| `box.get(key)` | `void box_get(key)(...)` | Essential operation for managing `box.get(key)` within the Flutter framework. |
| `box.put(key, value)` | `void box_put(key, value)(...)` | Essential operation for managing `box.put(key, value)` within the Flutter framework. |
| `@HiveType` | `void HiveType(...)` | Essential operation for managing `@HiveType` within the Flutter framework. |
| `@HiveField` | `void HiveField(...)` | Essential operation for managing `@HiveField` within the Flutter framework. |
| `TypeAdapter` | `void TypeAdapter(...)` | Essential operation for managing `TypeAdapter` within the Flutter framework. |
| `Hive.registerAdapter()` | `void Hive_registerAdapter(...)` | Essential operation for managing `Hive.registerAdapter()` within the Flutter framework. |
| `Hive.initFlutter()` | `void Hive_initFlutter(...)` | Essential operation for managing `Hive.initFlutter()` within the Flutter framework. |
...
To further understand the intricacies of Local Databases: SQLite with Drift and Hive, let's examine additional architectural patterns.
